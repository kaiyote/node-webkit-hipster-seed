...
exports['E2E App Tests'] =
  ...
  'Should list two items': (test) ->
    driver.findElements(webdriver.By.css '#routeHolder ul li').then (elements) ->
      test.equal 2, elements.length
      do test.done
      
  'Should display completed items with a line-through': (test) ->
    driver.findElement(webdriver.By.css '#routeHolder ul li input:checked + span').getCssValue('text-decoration').then (value) ->
      test.ok value.indexOf 'line-through' > -1
      do test.done